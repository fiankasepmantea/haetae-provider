...
// implement crypto_sign_keypair, crypto_sign_signature, crypto_sign_verify here
 // or whatever header contains haetae_sign, haetae_keypair, etc.
/*************************************************
...
static int haetae_openssl_sign(void *vctx,
                                unsigned char *sig, size_t *siglen, size_t sigsize,
                                const unsigned char *tbs, size_t tbslen)
{
    HAETAE_CTX *ctx = vctx;

    if (!ctx || !ctx->privkey || !siglen) return 0;

    if (sig && sigsize < HAETAE_SIGNATURE_LENGTH) return 0;

    return crypto_sign_signature(sig, siglen, tbs, tbslen, ctx->privkey);
}
